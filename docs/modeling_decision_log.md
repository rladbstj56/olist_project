...
Track B는 주문 시점에 이미 알 수 있는 정보만 사용해 부정 리뷰 위험을 예측하는 모델이다. 현재 `notebooks/03_ml_classifier.ipynb`의 `pre_order_cols` 기준으로 `expected_delivery_days`도 Track B feature에 포함되어 있다.

따라서 Track B는 다음처럼 해석한다.

```text
판매자 또는 플랫폼이 설정한 기존 예상 배송일을 포함해,
해당 주문이 부정 리뷰로 이어질 위험이 높은지 주문 직후 판단하는 모델
```
...
현재 Track B는 부정 리뷰 위험을 예측하지만, 위험 주문에 대해 “예상 배송일을 며칠로 제안해야 하는가”를 직접 답하지는 않는다. 이 질문은 별도의 목표 변수를 갖는 Track C로 분리하는 것이 적절하다.
...
Track C는 현재의 부정 리뷰 분류 모델과 목표가 다르므로, `review_score`나 부정 리뷰 여부를 직접 타깃으로 두기보다 실제 배송 소요일을 예측하는 모델로 설계하는 것이 자연스럽다.
...
90% 분위수를 기본안으로 둔 이유는 Track C의 목적이 평균 배송일 예측이 아니라 고위험 주문의 지연 tail risk를 줄이는 데 있기 때문이다. 평균 예측값은 “보통의 주문”을 설명하는 데는 유용하지만, 부정 리뷰로 이어질 가능성이 큰 늦은 배송 케이스를 충분히 보호하지 못할 수 있다.
...
따라서 Track C 구현 시 90% 분위수를 기본값으로 사용하되, 결과표에는 분위수별 `4일 이상 지연율`, `평균 추천 조정일수`, `현재 예상 배송일 대비 증가 폭`을 함께 비교하는 것이 바람직하다.
...
| 판정 | 부정 리뷰 위험 확률 | 운영 의미 | Track C 적용 |
| --- | ---: | --- | --- |
| 고위험 | 47% 초과 | 판매자가 별도 확인해야 할 주문 | 적용 |
| 주의 | 35% 초과부터 47% 이하 | 배송 상태 모니터링 대상 | 미적용 |
| 일반 | 35% 이하 | 일반 처리 | 미적용 |

47% 기준은 Track B에서 validation 기준으로 선택한 긍정 리뷰 확률 threshold `0.53`을 부정 리뷰 위험 확률로 뒤집은 값이다.

```text
부정 리뷰 위험 확률 = 1 - 긍정 리뷰 확률
고위험 기준 = 1 - 0.53 = 0.47
```
...
- 고위험 기준은 CS 처리 가능 인력, 판매자 확인 가능량, 부정 리뷰 미탐 비용에 따라 달라질 수 있다.
...
| 기준 | 기본값 | 근거 |
| --- | ---: | --- |
| Track B 고위험 기준 | 47% | 검증셋에서 선택한 긍정 리뷰 threshold `0.53`을 부정 리뷰 위험 확률로 변환 |
...
> 운영자가 CS 리소스와 고객 경험 전략에 따라 부정 리뷰 위험 기준과 예상 배송일 보수성을 직접 조정할 수 있는 의사결정 지원 UI를 구현했다. 기본값은 검증셋 threshold와 90% 분위수 운영 가정에 기반하되, 실제 운영에서는 기준별 지연 감소 효과와 예상 배송일 증가 폭을 비교해 조정할 수 있다.
